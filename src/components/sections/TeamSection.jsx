...
export default function TeamSection({ preview = false }) {
  ...
  return (
    <section style={{ background:'var(--lavelle-ivory)', padding:'48px var(--space-xl)' }}>
      <div className="container">
        <SectionHeader eyebrow={t('team.eyebrow')} headline={t('team.headline')} subtext={t('team.sub')} align="center" />
        <div className="tile-grid" style={{ gridTemplateColumns:'repeat(auto-fit, minmax(280px, 360px))', gap:'var(--space-2xl)' }}>
          {members.map(member => (
            (() => {
            ...
            <div key={member.name} data-image-slot={`team-${member.name.toLowerCase()}`}
              ...
              <div style={{ padding:'var(--space-xl)' }}>
                ...
                <p style={{ fontFamily:'var(--font-body)', fontSize:'var(--text-micro)', fontWeight:500, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--lavelle-gold-champagne)', marginBottom:'var(--space-md)' }}>{displayTitle}</p>
                <p style={{ fontFamily:'var(--font-body)', fontSize:'var(--text-small)', color:'var(--lavelle-gray-mid)', lineHeight:1.7 }}>{displayBio}</p>
                {displayQuote && <p style={{ fontFamily:'var(--font-display)', fontStyle:'italic', fontSize:'0.95rem', color:'var(--lavelle-plum-soft)', marginTop:'var(--space-md)', lineHeight:1.6 }}>{displayQuote}</p>}
              </div>
            </div>
              )
            })()
