...
export default function NewsletterSignup() {
  ...
  return (
    <section style={{ background:'var(--lavelle-plum-mid)', padding:'12px var(--space-sm)', textAlign:'center' }}>
      <div style={{ maxWidth:'520px', margin:'0 auto' }}>
        ...
        <p style={{ fontFamily:'var(--font-body)', fontWeight:300, fontSize:'var(--text-body)', color:'rgba(255,255,255,0.7)', marginBottom:'var(--space-lg)', lineHeight:1.7 }}>{t('newsletter.sub')}</p>
        {joined ? (
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', gap:'var(--space-sm)', flexWrap:'wrap', justifyContent:'center' }}>
            <input type="email" required placeholder={t('newsletter.placeholder')} value={email} onChange={e => setEmail(e.target.value)} aria-label={t('a11y.email')}
