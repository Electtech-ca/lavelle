...
export default function BookingModal({ isOpen, onClose, defaultService = '' }) {
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(49,58,77,0.75)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'var(--space-md)', backdropFilter:'blur(4px)' }}>
      <div onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-label={t('booking.dialogLabel')}
        ...
        <button onClick={onClose} aria-label={t('a11y.close')} style={{ position:'absolute', top:'var(--space-lg)', right:'var(--space-lg)', background:'var(--lavelle-plum-whisper)', border:'none', borderRadius:'50%', width:'36px', height:'36px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', color:'var(--lavelle-plum-mid)' }}>
          <X size={16} />
        </button>
