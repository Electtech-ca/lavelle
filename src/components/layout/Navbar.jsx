...
export default function Navbar() {
  ...
  const handleTabSelection = event => {
    ...
    navigateToTop(`${url.pathname}${url.search}${url.hash}`)
  }
  ...
  const searchItems = [
    { path: '/', label: t('nav.home'), keywords: 'home accueil' },
  ...
  const searchResults = normalizedQuery
    ? searchItems.filter(item => `${item.label} ${item.keywords}`.toLocaleLowerCase().includes(normalizedQuery))
    : searchItems
    ...
    <>
          ...
          <ul style={{ display: 'flex', gap: 'var(--space-lg)', listStyle: 'none', margin: '0 auto', padding: 0, alignItems: 'center' }} className="nav-desktop">
            {navKeys.map(l => (
              <li key={l.to} style={{ whiteSpace: 'nowrap' }}>
                <NavLink to={l.to} end={l.to === '/'} onClick={handleTabSelection} style={linkStyle}>{t(l.key)}</NavLink>
              </li>
            ))}
          </ul>
      ...
      {searchOpen && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, zIndex: 600, background: 'rgba(46,51,80,0.72)', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: 'clamp(96px, 14vh, 150px) var(--space-lg) var(--space-lg)' }}>
          <div style={{ width: 'min(680px, 100%)', maxHeight: 'min(620px, calc(100vh - 130px))', overflow: 'auto', background: 'var(--color-white)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-modal)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)', padding: 'var(--space-lg)', borderBottom: '1px solid var(--color-cream)' }}>
              <Search size={20} color="var(--color-blue)" aria-hidden="true" />
              <input
                ref={searchInputRef}
                type="search"
                value={searchQuery}
                ...
                style={{ flex: 1, minWidth: 0, border: 0, outline: 0, background: 'transparent', color: 'var(--color-blue)', fontFamily: 'var(--font-body)', fontSize: '1rem' }}
              />
              <button onClick={() => setSearchOpen(false)} aria-label={t('modal.close')} style={{ color: 'var(--color-blue)', padding: '4px' }}>
                <X size={20} />
              </button>
            </div>
            <div style={{ padding: 'var(--space-md)' }}>
              <p style={{ padding: 'var(--space-sm) var(--space-md)', color: 'var(--color-blue-55)', fontFamily: 'var(--font-body)', fontSize: 'var(--text-micro)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>
                {normalizedQuery ? t('search.results') : t('search.browse')}
              </p>
              {searchResults.length > 0 ? searchResults.map(result => (
                <button key={result.path} onClick={() => goToSearchResult(result.path)} style={{ display: 'block', width: '100%', padding: 'var(--space-md)', textAlign: 'left', color: 'var(--color-blue)', background: 'transparent', borderRadius: 'var(--radius-md)', fontFamily: 'var(--font-body)', fontSize: 'var(--text-small)' }}>
                  {result.label}
                </button>
              )) : (
                <p style={{ padding: 'var(--space-md)', color: 'var(--color-blue-55)', fontFamily: 'var(--font-body)' }}>{t('search.empty')}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          {navKeys.map(l => (
            <NavLink
              ...
              onClick={event => { setMenuOpen(false); handleTabSelection(event) }}
              style={({ isActive }) => ({
