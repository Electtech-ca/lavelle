...
import { brunchTranslations, afternoonTeaTranslations, alaCarteTranslations } from '../data/gourmetTranslations'
...
function FoodCard({ item, img, translation }) {
    ...
    <div style={{ background: 'var(--lavelle-white)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', boxShadow: 'var(--shadow-card)', transition: 'transform 0.3s ease, box-shadow 0.3s ease' }}
      ...
      <div style={{ padding: 'var(--space-xl)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 'var(--space-md)', marginBottom: 'var(--space-sm)' }}>
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-h3)', fontWeight: 400, color: 'var(--lavelle-plum-deep)', lineHeight: 1.3 }}>{displayName}</h3>
        ...
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-small)', color: 'var(--lavelle-gray-mid)', lineHeight: 1.75 }}>{displayDescription}</p>
      </div>
    </div>
  )
}
...
export default function GourmetFood() {
  ...
  return (
    <>
      ...
      <div style={{ background: 'var(--lavelle-ivory)', padding: 'var(--space-lg) var(--space-lg) var(--space-sm)', minHeight: '60vh' }}>
        <div className="container">
          ...
          {tab === 0 && (
            <>
              ...
              <div className="gourmet-menu-grid" style={{ gap: 'var(--space-xl)', marginBottom: 'var(--space-lg)' }}>
                {brunchItems.map((item, i) => (
                  <FoodCard key={item.id} item={item} translation={isFrench ? brunchTranslations[item.id] : null} img={BRUNCH_IMGS[i % BRUNCH_IMGS.length]} />
                ))}
              </div>
              <div style={{ padding: 'var(--space-md)', background: 'var(--lavelle-plum-whisper)', borderRadius: 'var(--radius-xl)', display: 'flex', gap: 'var(--space-xl)', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {tab === 1 && (
            <>
              ...
              {/* Hero tea image */}
              <div className="tile-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 380px))', gap: 'var(--space-xl)', marginBottom: 'var(--space-2xl)' }}>
                {afternoonTea.map((item, i) => (
                  (() => {
                    const translation = isFrench ? afternoonTeaTranslations[item.id] : null
                    const displayName = translation?.name || item.name
                    const displayDescription = translation?.description || item.description
                    return (
                  <div key={item.id} style={{
                    ...
                    <div style={{ height: '220px', overflow: 'hidden' }}>
                      <img src={TEA_IMGS[i % TEA_IMGS.length]} alt={displayName} loading="lazy"
                    ...
                    <div style={{ padding: 'var(--space-xl)' }}>
                      ...
                      <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', fontWeight: 400, color: item.id === 3 ? 'var(--lavelle-white)' : 'var(--lavelle-plum-deep)', marginBottom: 'var(--space-sm)' }}>{displayName}</h3>
                      ...
                      <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-small)', color: item.id === 3 ? 'rgba(255,255,255,0.75)' : 'var(--lavelle-gray-mid)', lineHeight: 1.75, marginBottom: 'var(--space-lg)' }}>{displayDescription}</p>
          ...
          {tab === 2 && (
            <>
              ...
              <div className="gourmet-menu-grid" style={{ gap: 'var(--space-xl)', marginBottom: 'var(--space-2xl)' }}>
                {alaCarteItems.map((item, i) => (
                  <FoodCard key={item.id} item={item} translation={isFrench ? alaCarteTranslations[item.id] : null} img={ALACARTE_IMGS[i % ALACARTE_IMGS.length]} />
                ))}
              </div>
